...
원하는 데이터가 temp의 데이터보다 크면 오른쪽 자식으로 가서 탐색을 다시합니다 (새로운 temp로 반복문이 다시 돌아간다).

원하는 데이터가 temp의 데이터보다 작으면 왼쪽 자식으로 가서 탐색을 다시합니다(새로운 temp로 반복문이 다시 돌아간다).

반복문이 끝날 때까지 원하는 노드를 찾지 못했다는 건 트리 안에 원하는 데이터를 갖는 노드가 없다는 뜻이겠죠? 이때는 그냥 None을 리턴해줍니다."""
